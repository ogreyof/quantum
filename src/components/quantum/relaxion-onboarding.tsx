...
interface RelaxionOnboardingProps {
  onComplete: (answers: RelaxionAnswers) => void;
}

export interface RelaxionAnswers {
  focus: string;
  schedule: string;
  routine: string[];
}

export function RelaxionOnboarding({ onComplete }: RelaxionOnboardingProps) {
  const [currentStep, setCurrentStep] = useState(0);
  const [answers, setAnswers] = useState<RelaxionAnswers>({
    focus: '',
    schedule: '',
    routine: []
  });
  ...
  const steps = [
    {
      title: "Bem-vindo ao Relaxion Plus! 🎉",
      subtitle: "Vamos personalizar sua experiência em 3 passos",
      content: (
        <div className="text-center space-y-6">
          ...
          <p className="text-gray-300">
            Em apenas 3 perguntas, vamos criar sua rotina diária personalizada de 15 minutos.
          </p>
        </div>
      )
    },
    {
      ...
      content: (
        <div className="space-y-3">
          {[
            { id: 'cervical', label: 'Cervical/Tensões', icon: '🦴', desc: 'Alívio de dores no pescoço e tensões' },
            { id: 'drenagem', label: 'Drenagem Facial/Inchaço', icon: '💧', desc: 'Reduzir inchaço e melhorar contorno facial' },
            { id: 'capilar', label: 'Capilar (Fortalecimento)', icon: '💇', desc: 'Fortalecer folículos e estimular crescimento' },
    ...
    },
    {
      ...
      content: (
        <div className="space-y-3">
          {[
            { id: 'morning', label: 'Manhã (08:00)', icon: '🌅', desc: 'Comece o dia sem tensão' },
            { id: 'lunch', label: 'Almoço (12:30)', icon: '☀️', desc: 'Pausa inteligente no trabalho' },
            { id: 'evening', label: 'Noite (19:30)', icon: '🌙', desc: 'Relaxe após o dia de trabalho' }
          ].map((option) => (
            <button
              key={option.id}
    ...
    },
    {
      ...
      content: (
        <div className="space-y-3">
          {[
            { id: 'computer', label: 'Muitas horas no computador', icon: '💻', desc: 'Trabalho sedentário, tensão cervical' },
